...
export type TradeToastVariant = "success" | "error" | "info" | "warning";

export interface TradeToastPayload {
  message: string;
  variant?: TradeToastVariant;
  duration?: number;
}

interface TradeToastItem extends Required<Pick<TradeToastPayload, "message">> {
  id: string;
  variant: TradeToastVariant;
  duration: number;
}
...
type TradeToastContextValue = {
  showToast: (payload: TradeToastPayload | string, variant?: TradeToastVariant) => void;
};
...
export function useTradeToast() {
  ...
  if (!ctx) {
    throw new Error("useTradeToast must be used within TradeToastProvider");
  }
  return ctx;
}

export async function parseTradeApiError(
  res: Response,
  ...
  return (
    ...
    >
      ...
      <button
        ...
        className="flex size-9 shrink-0 cursor-pointer items-center justify-center rounded-full text-current/50 transition-colors hover:bg-black/5 hover:text-current"
...
export function TradeToastProvider({ children }: { children: React.ReactNode }) {
  ...
  const showToast = useCallback(
    (payload: TradeToastPayload | string, variant: TradeToastVariant = "success") => {
      const normalized: TradeToastPayload =
        typeof payload === "string" ? { message: payload, variant } : payload;

      const id = crypto.randomUUID();
      const item: TradeToastItem = {
        id,
  ...
  return (
    <TradeToastContext.Provider value={{ showToast }}>
      {children}
      <div
        id={regionId}
        aria-label="Alertas de trocas"
        ...
        <AnimatePresence mode="popLayout">
          {toasts.map((toast) => (
            <div key={toast.id} className="w-full max-w-md">
              <TradeToastCard toast={toast} onDismiss={dismiss} />
            </div>
          ))}
        </AnimatePresence>
      </div>
