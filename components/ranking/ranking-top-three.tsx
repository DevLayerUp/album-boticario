...
const PLACE_STYLES = {
  1: {
    ...
    avatar:
      "size-[104px] @md:size-[140px] @lg:size-[180px] @xl:size-[220px] @2xl:size-[257px]",
  },
  2: {
    order: "md:order-1",
    ...
    avatar:
      "size-[88px] @md:size-[120px] @lg:size-[160px] @xl:size-[190px] @2xl:size-[218px]",
  },
  3: {
    order: "md:order-3",
...
function PodiumPlace({
    ...
    return (
      ...
      >
        <div className={cn("relative pb-4", place === 1 && config.glowClass)}>
          <RankingAvatar
            entry={entry}
            sizeClassName={isFeatured ? "size-[112px]" : "size-[80px]"}
          ...
          <span
            className={cn(
              ...
              config.badge,
              isFeatured ? "px-5 py-1.5 text-xs" : "px-3 py-1 text-[10px]",
            )}
          >
            {config.label}
          </span>
        ...
        <div className="w-full min-w-0 space-y-0 text-center leading-[1.4]">
          <p
            className={cn(
              ...
              isFeatured ? "text-xl" : "truncate px-1 text-base",
            )}
            title={name}
          >
            {name}
          </p>
          <p className={cn("font-semibold text-amarelo", isFeatured ? "text-base" : "text-sm")}>
  ...
  return (
    <motion.div
      ...
      transition={{ delay: place * 0.06 }}
      className={cn(
        "flex min-w-0 flex-1 flex-col items-center gap-3 @md:gap-4 @xl:gap-[19px]",
        config.order,
      )}
    >
      ...
      <div className="w-full min-w-0 max-w-39 space-y-0 px-1 text-center leading-[1.4] @md:max-w-42 @xl:max-w-39">
        <p
          className="truncate font-display text-base font-bold text-white @md:text-xl @lg:text-2xl @xl:text-[34px]"
          title={name}
        >
          {name}
        </p>
        <p className="truncate text-sm font-semibold text-amarelo @md:text-lg @xl:text-xl">
          {formatRankingPoints(entry.score)} pts
        </p>
        <p className="truncate text-xs font-medium text-verde-300 @md:text-sm @xl:text-base">
          {entry.album_pct}% do álbum
        </p>
      </div>
    </motion.div>
...
export function RankingTopThree({ entries, updatedLabel }: RankingTopThreeProps) {
  ...
  return (
    <section className="@container/top3 relative overflow-hidden rounded-card bg-verde-escuro-500 @xl:h-[612px]">
      ...
      <div className="relative z-10 flex h-full min-h-0 flex-col px-4 pt-6 sm:px-6 sm:pt-8 @xl:px-10 @xl:pt-10">
        <div className="flex shrink-0 flex-col gap-2 sm:flex-row sm:items-center sm:justify-between sm:gap-3">
          <p className="text-sm font-semibold uppercase tracking-[0.14em] text-verde-300 @md:text-base @xl:text-xl">
            TOP 3
          </p>
          <p className="text-xs uppercase tracking-[0.12em] text-verde-300 sm:text-sm sm:tracking-[0.14em] @xl:text-xl">
            {updatedLabel}
          </p>
        </div>

        {/* Mobile */}
        <div className="mt-5 space-y-5 pb-6 md:hidden">
        ...
        <div className="hidden min-h-0 flex-1 flex-col justify-end pb-6 pt-6 md:flex @xl:pb-10 @xl:pt-8">
          <div className="flex w-full items-end justify-center gap-3 px-1 @md:gap-5 @lg:gap-8 @xl:gap-[49px] @xl:px-[75px]">
